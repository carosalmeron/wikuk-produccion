...
function PlanMesTab({ periodo, setPeriodo, plan, guardar, productos, mps, semanas, planesSem, slotsDia=SLOTS_DIA, persLinea=3, persDia=12, turnosCentro=TURNOS_ABIERTOS, centroNombre="", perfil, ggMes=0, irReparto }) {
  ...
  const critGG = plan.reparto_gg || "capacidad";  // capacidad · ventas · turnos · unidades
  ...
  const ggAbsorbido = () => items.reduce((a,it) =>
    a + ggDeProducto(productos.find(x=>x.id===it.producto_id), toNum(it.cantidad)), 0);

  // Gastos generales que le tocan a un producto, según el criterio elegido
  // Capacidad del mes si la fábrica entera hiciera solo ese producto
  const capacidadDe = (p) => {
    const ritmo = toNum(p?.uds_turno_linea), pers = parseInt(p?.personas_linea)||3;
    if (ritmo <= 0) return 0;
    const ancho = Math.max(1, Math.ceil(pers/persLinea));
    return ritmo * Math.floor(slotsEfect/ancho) * dias;
  };
  const ggDeProducto = (p, q) => {
    if (!ggMes || !q) return 0;
    const ritmo = toNum(p?.uds_turno_linea), pers = parseInt(p?.personas_linea)||3;
    if (critGG === "capacidad") {
      const cap = capacidadDe(p);
      return cap > 0 ? (ggMes / cap) * q : 0;
    }
    if (critGG === "ventas") {
      const v = toNum(p?.precio_venta) * q;
      return r.ventas > 0 ? ggMes * (v / r.ventas) : 0;
    }
    if (critGG === "turnos") {
      const sl = ritmo>0 ? (q/ritmo) * Math.max(1, Math.ceil(pers/persLinea)) : 0;
      return r.slots > 0 ? ggMes * (sl / r.slots) : 0;
    }
    return r.uds > 0 ? ggMes * (q / r.uds) : 0;   // por unidades
  };
  ...
  const imprimir = () => {
    ...
    const critTxt = critGG==="capacidad" ? "por capacidad productiva de cada producto"
      : critGG==="ventas" ? "por ventas" : critGG==="turnos" ? "por turnos de línea" : "por unidades";
    const ggAbs = ggAbsorbido(), ggOci = ggMes - ggAbs;
    const economia = items.map(it=>{
      ...
      return `<tr${rojo?' style="background:#FCEBEB"':''}>
        ...
        <td class="n">${num(q)}</td>
        <td class="n">${capacidadDe(p)>0?num(capacidadDe(p)):"—"}</td>
    ...
    imprimirHTML(`
      ...
      <table>
        <tr><th>Producto</th><th class="n">Uds</th><th class="n">Cap. mes con ${persProg}p</th><th class="n">Materia €/ud</th><th class="n">MO €/ud</th><th class="n">Generales €/ud</th><th class="n">Coste €/ud</th><th class="n">Venta €/ud</th><th class="n">Margen €/ud</th><th class="n">Margen %</th><th class="n">Beneficio</th></tr>
        ${economia}
        <tr><td><b>TOTAL</b></td><td class="n"><b>${num(r.uds)}</b></td><td class="n">—</td>
      ...
      <div style="font-size:9.5px;color:#666;margin-bottom:6px">
        Generales: los ${eur(ggMes)} del mes repartidos <b>${critTxt}</b>${critGG==="capacidad"?`; el plan absorbe ${eur(ggAbs)} y quedan ${eur(ggOci)} de capacidad ociosa`:""}.
      ...
      <table>
        ...
        <tr><td>Gastos generales del mes (completos, ${critTxt})</td><td class="n">${eur(ggMes)}</td><td class="n">${r.uds>0?(ggMes/r.uds).toFixed(2):"0.00"} medio</td></tr>
      ...
      <table>
        ...
        <tr><td>Gastos generales absorbidos por el plan</td><td class="n">${eur(ggAbs)}</td><td class="n">${r.ventas>0?Math.round(ggAbs/r.ventas*100):"—"}%</td></tr>
        <tr><td>Capacidad ociosa (generales sin absorber)</td><td class="n">${eur(ggOci)}</td><td class="n">${r.ventas>0?Math.round(ggOci/r.ventas*100):"—"}%</td></tr>
    ...
    <>
      ...
      {(() => {
        ...
        return (
          <>
            <Card style={{marginBottom:12}}>
              <div style={{fontFamily:F.h,fontWeight:800,fontSize:14,color:C.text,marginBottom:3}}>⚖️ Cómo se reparten los gastos generales</div>
              <div style={{fontSize:11.5,color:C.mutedD,lineHeight:1.55,marginBottom:9}}>
                Los {eur(ggMes)} se cargan enteros, pero hay que decidir cuánto le toca a cada producto.
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:9}}>
                {[["capacidad","Por capacidad","Lo que daría la fábrica de ese producto"],
                  ["ventas","Por ventas","Paga más el que más factura"],
                  ["turnos","Por turnos","Reparte los del plan por ocupación"],
                  ["unidades","Por unidades","Todos igual por ud"]].map(([k,t,d])=>(
                  <button key={k} onClick={()=>guardar({reparto_gg:k})}
                    style={{background:critGG===k?C.accent:"#fff",color:critGG===k?"#fff":C.text,
                      border:`1.5px solid ${critGG===k?C.accent:C.border}`,borderRadius:11,padding:"10px 6px",
                      fontFamily:F.h,fontWeight:800,fontSize:12,cursor:"pointer",textAlign:"center"}}>
                    <div>{t}</div>
                    <div style={{fontSize:9.5,fontWeight:600,opacity:0.75,marginTop:2,lineHeight:1.25}}>{d}</div>
                  </button>
                ))}
              </div>
              {critGG==="capacidad" && (
                <div style={{background:C.blueBg,borderRadius:10,padding:"9px 11px",marginBottom:8,fontSize:11.5,color:C.text,lineHeight:1.6}}>
                  A cada producto le toca <b>{eur(ggMes)} ÷ lo que daría de él la plantilla programada</b>: {persProg} personas, {slotsEfect} huecos al día, {dias} días.
                  El coste por unidad no cambia aunque cambies el plan, solo si cambias la plantilla.
                  {(() => { const ab = ggAbsorbido(); const oc = ggMes - ab;
                    return oc > 1 ? <div style={{color:C.amber,fontWeight:700,marginTop:4}}>
                      Este plan absorbe {eur(ab)}. Los otros <b>{eur(oc)}</b> son capacidad ociosa: no los carga ningún producto, pero se pagan y restan del beneficio.
                    </div> : null; })()}
                </div>
              )}
              {items.length>0 && (
                <div style={{background:C.card2,borderRadius:10,padding:"9px 11px",fontSize:12,color:C.mutedD,lineHeight:1.7}}>
                  {items.slice(0,5).map(it=>{
                    const p = productos.find(x=>x.id===it.producto_id);
                    const q = toNum(it.cantidad);
                    const g = ggDeProducto(p, q);
                    ...
                    return (
                      <div key={it.id} style={{display:"flex",justifyContent:"space-between",gap:8}}>
                        ...
                        <b style={{flexShrink:0,color:C.text}}>{q>0?(g/q).toFixed(2):"0.00"} €/ud{pv>0 && <span style={{fontWeight:400,color:C.muted}}> · {Math.round(g/q/pv*100)}% del precio</span>}</b>
                      </div>
                    );
                  })}
                  {items.length>5 && <div style={{fontSize:11}}>…y {items.length-5} más</div>}
                </div>
              )}
            </Card>
            ...
            <Card style={{marginBottom:12}}>
              ...
              {fila("Gastos generales del mes", eur(ggMes),
                critGG==="capacidad"?`${eur(ggAbsorbido())} absorbidos · ${eur(ggMes-ggAbsorbido())} ociosos`
                :critGG==="ventas"?"repartidos por ventas":critGG==="turnos"?"repartidos por turnos de línea":"repartidos por unidades")}
...
// Tabla: una fila por producto, una casilla por semana.
function RepartoTab({ periodo, semanas, planMes, planesSem, productos, moldes=[],
                      slotsDia=SLOTS_DIA, persLinea=3, centroId="", irASemana, irAlMes }) {
  ...
  const capSemana = slotsDia * 5;

  useEffect(() => {
    const d = {};
    semanas.forEach(s => {
      d[s] = {};
      (planesSem.find(p => p.semana === s)?.items || []).forEach(it => {
        d[s][it.producto_id] = (d[s][it.producto_id]||0) + toNum(it.cantidad);
      });
    });
  ...
  const udsQueCaben = (pid, sl) => ritmoDe(pid) > 0 ? (sl / anchoDe(pid)) * ritmoDe(pid) : 0;
  ...
  const setQ = (s, pid, v) => { setGuardado(false);
    setDraft(p => ({ ...p, [s]: { ...(p[s]||{}), [pid]: v==="" ? "" : Math.max(0, toNum(v)) } })); };

  // reparte a partes iguales lo que falte de un producto
  const autoUno = (it) => {
    if (!abiertas.length) return;
    const falta = pendiente(it);
    if (Math.abs(falta) < 0.5) return;
    const trozo = Math.round(falta/abiertas.length);
    setGuardado(false);
    setDraft(p => {
      const n = { ...p };
      abiertas.forEach((s,i) => {
        const base = toNum(n[s]?.[it.producto_id]);
        const add = i === abiertas.length-1 ? falta - trozo*(abiertas.length-1) : trozo;
        n[s] = { ...(n[s]||{}), [it.producto_id]: Math.max(0, Math.round(base + add)) };
      });
      return n;
    });
  };
  ...
  // propuesta completa: carga pareja y moldes juntos
  const proponer = () => {
    if (!items.length) return;
    if (!abiertas.length) { window.alert("Todas las semanas del mes están cerradas."); return; }
    if (items.every(it => ritmoDe(it.producto_id) <= 0)) {
      window.alert("Ningún producto tiene ritmo definido. Ponlo en su ficha."); return; }
    if (!window.confirm("Reparto el mes solo: carga pareja entre semanas, sin pasarme de la capacidad y juntando los del mismo molde.\n\nSe sobrescribe el reparto de las semanas abiertas. ¿Seguir?")) return;

    const claves = [...new Set(items.map(it => moldeKey(it.producto_id)))];
    const cola = [...items].filter(it => ritmoDe(it.producto_id) > 0)
      .sort((a,b) => {
        const ka = moldeKey(a.producto_id), kb = moldeKey(b.producto_id);
        if (ka !== kb) return claves.indexOf(ka) - claves.indexOf(kb);
        return toNum(b.cantidad) - toNum(a.cantidad);
      }).map(it => ({ pid: it.producto_id, pend: toNum(it.cantidad) }));

    const d = {}; semanas.forEach(s => { d[s] = cerradas(s) ? { ...(draft[s]||{}) } : {}; });
    const cargaDe = (s) => items.reduce((a,it) => a + slotsDe(it.producto_id, d[s][it.producto_id]||0), 0);
    const total = cola.reduce((a,x) => a + slotsDe(x.pid, x.pend), 0);
    const objetivo = Math.min(capSemana, total / abiertas.length);

    let i = 0;
    for (const paso of [objetivo, capSemana]) {
      for (const s of abiertas) {
        let cap = paso - cargaDe(s);
        while (i < cola.length && cap > 0.05) {
          const x = cola[i];
          if (x.pend <= 0.5) { i++; continue; }
          const nec = slotsDe(x.pid, x.pend);
          if (nec <= cap + 0.05) { d[s][x.pid] = (d[s][x.pid]||0) + Math.round(x.pend); cap -= nec; x.pend = 0; i++; }
          else { const u = Math.floor(udsQueCaben(x.pid, cap)); if (u >= 1) { d[s][x.pid] = (d[s][x.pid]||0) + u; x.pend -= u; } cap = 0; }
        }
      }
      i = 0;
    }
    ...
    if (fuera.length) window.alert("No cabe todo en el mes:\n\n" +
      fuera.map(x => `· ${prodDe(x.pid)?.nombre}: ${num(x.pend)} uds sin colocar`).join("\n"));
  };

  const vaciar = () => {
    if (!window.confirm("¿Poner a cero todo el reparto?")) return;
    setGuardado(false);
  ...
  return (
    <>
      <Card style={{marginBottom:12}}>
        <div style={{fontFamily:F.h,fontWeight:800,fontSize:14,color:C.text,marginBottom:3}}>✂️ Reparto en semanas</div>
        <div style={{fontSize:12.5,color:C.mutedD,lineHeight:1.6,marginBottom:11}}>
        ...
        <button onClick={proponer}
          style={{width:"100%",background:C.accent,border:"none",color:"#fff",borderRadius:12,padding:"15px",
            fontFamily:F.h,fontWeight:800,fontSize:15,cursor:"pointer",marginBottom:8}}>⚡ Repartir el mes solo</button>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <button onClick={autoTodo} style={{background:C.blueBg,border:`1.5px solid ${C.blue}55`,color:C.blue,borderRadius:11,padding:"12px",fontFamily:F.h,fontWeight:800,fontSize:13,cursor:"pointer"}}>⚖️ A partes iguales</button>
          <button onClick={vaciar} style={{background:"#fff",border:`1.5px solid ${C.border}`,color:C.mutedD,borderRadius:11,padding:"12px",fontFamily:F.h,fontWeight:800,fontSize:13,cursor:"pointer"}}>↺ Empezar de cero</button>
        </div>
      </Card>

      {items.map(it => {
        const p = prodDe(it.producto_id);
        const falta = pendiente(it);
        const ok = Math.abs(falta) < 0.5;
        return (
          <Card key={it.id} style={{marginBottom:10}} color={ok?C.green+"55":C.amber+"66"}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:9}}>
              <div style={{minWidth:0}}>
                ...
                <div style={{fontSize:11.5,color:C.mutedD,marginTop:2}}>
                  {ritmoDe(it.producto_id)>0
                    ? <>⏱️ {num(ritmoDe(it.producto_id))} uds/turno</>
                    : <span style={{color:C.red,fontWeight:700}}>⚠️ sin ritmo</span>}
                  {" · 🔧 "}{moldeNom(moldeKey(it.producto_id))}
                </div>
                <div style={{fontSize:11.5,color:C.mutedD,marginTop:1}}>Plan del mes: {num(it.cantidad)} uds</div>
              </div>
              <span style={{flexShrink:0,background:ok?C.greenBg:C.amberBg,border:`1.5px solid ${ok?C.green:C.amber}`,color:ok?C.green:C.amber,
                borderRadius:20,padding:"5px 12px",fontFamily:F.h,fontWeight:800,fontSize:12.5}}>
              ...
              {semanas.map(s => {
                ...
                return (
                  <div key={s}>
                    ...
                    <input type="text" inputMode="numeric" disabled={bloq}
                      value={draft[s]?.[it.producto_id] ?? ""}
                      onChange={e=>setQ(s, it.producto_id, e.target.value.replace(/[^0-9]/g,""))}
                      style={{width:"100%",padding:"12px 4px",borderRadius:10,border:`1.5px solid ${C.border}`,
                        background:bloq?C.card2:"#fff",color:bloq?C.muted:C.text,fontSize:16,fontFamily:F.h,fontWeight:700,
            ...
            {!ok && (
              <button onClick={()=>autoUno(it)}
                ...
                ⚖️ Repartir {falta>0?`estas ${num(falta)} uds`:`quitar ${num(-falta)} uds de más`}
              </button>
            )}
          </Card>
        );
      ...
      <Card style={{marginBottom:12}}>
        ...
        {semanas.map(s => {
          ...
          const ms = [...new Set(items.filter(it => enSemana(s,it.producto_id)>0).map(it => moldeKey(it.producto_id)))];
          return (
            <div key={s} onClick={()=>irASemana(s)} style={{padding:"10px 0",borderBottom:`1px solid ${C.card2}`,cursor:"pointer"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{minWidth:0,flex:1}}>
                  <div style={{fontFamily:F.h,fontWeight:800,fontSize:14,color:C.text}}>{cerradas(s)?"🔒 ":""}Semana {s.split("-W")[1]}</div>
                  <div style={{fontSize:11.5,color:C.mutedD}}>{rotuloSemana(s)}{ms.length?` · 🔧 ${ms.map(moldeNom).join(" · ")}`:""}</div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontFamily:F.h,fontWeight:900,fontSize:17,color:col}}>{Math.round(pct*100)}%</div>
                  <div style={{fontSize:10,color:C.mutedD}}>{Math.ceil(carga/5*persLinea)} pers./día</div>
                </div>
                <span style={{color:C.muted,fontSize:17,flexShrink:0}}>›</span>
              </div>
              <div style={{height:7,background:C.card2,borderRadius:4,overflow:"hidden",marginTop:6}}>
